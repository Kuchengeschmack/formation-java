...
public class ClientDaoImpl implements IClientDao {
			...
			if (rs != null) {
				...
				while (rs.next()) {
					Client clientOut = new Client(rs.getInt("id"), rs.getString("courriel"), rs.getString("mdp"),
							...
							rs.getInt("code_postal"), rs.getString("ville"), rs.getInt("telephone"));
					liste.add(clientOut);
				}
	...
	public int ajouterClient(Client client) {
		try {
			...
			String req = "INSERT INTO clients (courriel, mdp, nom, prenom, adresse, code_postal, ville, telephone) values (?,?,?,?,?,?,?,?)";
			// Récupérer le ps
			ps = cx.prepareStatement(req);
		...
		try {
			...
			if (rs.next()) {
				...
				Client clientOut = new Client(rs.getInt("id"), rs.getString("courriel"), rs.getString("mdp"),
						rs.getString("nom"), rs.getString("prenom"), rs.getString("adresse"), rs.getInt("code_postal"),
						rs.getString("ville"), rs.getInt("telephone"));
				clientOut.setCompteCourant(compteCourantService.getCompteById(clientOut,
						new CompteCourant(rs.getInt("compte_courant_id"))));
				clientOut.setCompteEpargne(compteEpargneService.getCompteById(clientOut,
						new CompteEpargne(rs.getInt("compte_epargne_id"))));
	...
	public int deleteClientById(Client client) {
		try {
			...
			// Passage des paramètres
			ps.setInt(1, client.getId()); // On récupère l'identifiant du formateur
